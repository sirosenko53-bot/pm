...
export const mergeMembers = (
  ...
  return [...baseMembers, ...customMembers].filter((member) => {
    if (hiddenIds.has(member.memberId)) return false;
...
export const addCustomMember = (
  ...
  const hiddenBaseMember = baseMembers.find((member) =>
    hiddenIds.has(member.memberId) && normalizeMemberName(member.displayName) === name,
  );
  ...
  const existingMembers = mergeMembers(baseMembers, currentCustomMembers, currentHiddenMemberIds);
